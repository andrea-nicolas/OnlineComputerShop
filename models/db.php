...
class mydb
{
    ...
    function searchAndFilterProducts($q, $cat, $brand, $maxPrice, $conn)
    {
        $sql = "SELECT p.* FROM products p 
    ...
    function checkCartItem($userId, $productId, $conn)
    {
    ...
    function updateCartQuantity($userId, $productId, $quantity, $conn)
    {
        $sql = "UPDATE carts SET quantity = $quantity WHERE user_id = $userId AND product_id = $productId";
        return $conn->query($sql);
    }

    function deleteCartItem($userId, $productId, $conn)
    {
    ...
    {
        $sql = "SELECT c.id AS cart_id, c.quantity, p.id AS product_id, p.name, p.price, p.image_path, p.stock 
                FROM carts c 
    ...
    {
        $sql = "SELECT SUM(quantity) AS total_count FROM carts WHERE user_id = $userId";
